...
function Inventory() {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const response = await fetch(
                `${config.apiUrl}/api/swalook/inventory/product/?id=${productId}`,
                {
                    method: "DELETE",
                    headers: {
    ...
    const groupedInventory = inventoryData.reduce((acc, item) => {
        const category =
            item.category_details?.product_category || "Uncategorized";
                ...
                <div className="overflow-x-auto ml-0 md:ml-[19rem]">
                    {Object.keys(groupedInventory).map((category) => (
                        <div key={category} className="mb-8">
                            <h2 className="text-xl font-semibold  py-2 px-4 rounded-t-lg">
                                {category}
                            </h2>
