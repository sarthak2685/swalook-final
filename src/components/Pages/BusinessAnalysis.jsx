...
function BusinessAnalysis() {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [token, currentWeek, currentMonth, currentYear, selectedPeriod, bid]);
    ...
    const donutChartData = useMemo(() => {
        ...
        const labels = filteredServices.map(
            (service) =>
                `${service.label}: Rs. ${service.value.toLocaleString()}`
        );
    ...
    const fetchSalesPerData = async () => {
        ...
        try {
            ...
            if (data.status) {
                ...
                const employeeNames = sales_by_staff.map(
                    (item) => item.service_by
                );
                const employeeSales = sales_by_staff.map(
                    (item) => item.total_sales
                );

                // Prepare the sales per customer data
                const customerNames = billing_by_customer.map(
                    (item) => item.customer_name
                );
                const customerSales = billing_by_customer.map(
                    (item) => item.total_billing
                );
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (
                    data?.daily_product_analysis &&
                    productTimePeriod === "daily"
                ) {
                    setProductData(data.daily_product_analysis);
                } else if (
                    data?.monthly_product_analysis &&
                    productTimePeriod === "monthly"
                ) {
                    setProductData(data.monthly_product_analysis);
                } else {
                    console.error(
                        "Invalid response structure or no data:",
                        data
                    );
                }
            } catch (error) {
    ...
    return (
        <>
            <div className="bg-gray-100">
                <Header />
                <VertNav />
                <div className="bg-gray-100 flex-grow md:ml-72 lg:ml-72 p-4 md:p-6 lg:p-10">
