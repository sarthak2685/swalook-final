...
import { MdNotifications, MdHelpOutline, MdSettings } from "react-icons/md";
...
function Header() {
    ...
    const fetchInventory = async () => {
        try {
            ...
            const res = await axios.get(
                `${config.apiUrl}/api/swalook/inventory/expiring-products/?branch_name=${bid}`,
                {
                    headers: { Authorization: `Token ${token}` },
                }
            );
            const expiringProducts = res.data.expiring_products || [];
            const productNotifications = expiringProducts.map(
                (product) =>
                    `${product.product_name} is expiring on the ${product.expiry_date}.`
            );
    ...
    const handleService = () => navigate(`/${sname}/${branchName}/service`);
            ...
            >
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className="md:hidden mx-4 p-2 block bg-white"
                    >
                        <FiMenu
                ...
                <div className="flex items-center gap-6 mx-4">
                    ...
                    <div className="relative" ref={dropdownRef}>
                        <div
                            className="cursor-pointer py-2 px-4 bg-gray-100 rounded-full border flex items-center gap-2"
