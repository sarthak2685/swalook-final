...
function GenerateInvoice() {
        ...
        try {
            ...
            result.data.forEach((product) => {
                ...
                categoryMap.get(categoryName).products.push({
                    ...
                    stock: product.stocks_in_hand, // Added stock information
                    unit: product.unit || "", // Added unit information
                    expiryDate: product.expiry_date, // Added expiry date
                });
            });
    ...
    const toggleServiceSelection = (service) => {
        ...
        if (isSelected) {
            setSelectedList(selectedList.filter((s) => s.id !== service.id));
        } else {
    ...
    const updateServicesTableData = (updatedValues) => {
        ...
        console.log("newtabel data", newTableData);
    ...
    const fetchSpecificSerial = async () => {
        try {
            ...
            if (response?.data?.slno) {
                setInvoiceId(response.data.slno); // ✅ fine for UI
                return response.data.slno; // ✅ this is what you use
            } else {
            ...
            const response = await axios.post(
                `${config.apiUrl}/api/swalook/loyality_program/customer/?branch_name=${branchId}`,
                {
                    ...
                    membership: selectMembership
                        ? [
                              {
                                  program_type: selectMembership.program_type,
                              },
                          ]
                        : [],
                    d_o_b: dateOfBirth || "",
    ...
    const handlePhoneBlur = async () => {
        ...
        try {
            ...
            const apiEndpoint = `${config.apiUrl}/api/swalook/get-customer-bill-app-data/2/?branch_name=${branchName}&mobile_no=${mobile_no}`;
            ...
            if (
                ...
                setUserId(userData.id || ""); // It's UUID now, not numeric
                setUserExists(true);
                setDateOfBirth(userData.d_o_b || appointment?.d_o_b || "");
                setAnniversaryDate(userData.d_o_a || appointment?.d_o_a || "");

                // Membership type handled properly now
                setMembershipType(
                    userData.membership_type?.program_type || "None"
                );

                // Loyality points handled safely
                setUserPoints(
                    userData.loyality_profile?.current_customer_points || "0.00"
                );

                // Handle Coupons (if any)
                if (
                    ...
                    const userCoupons = userData.coupon.map((coupon) => ({
                        ...
                        couponPrice: coupon.coupon_name?.coupon_price || 0,
                        couponPointsHold:
                            coupon.coupon_name?.coupon_points_hold || 0,
    ...
    const { appointment, invoiceData, isEditMode } = location.state || {};

    // Set initial values from the data
    useEffect(() => {
        if (appointment) {
            // Handle data coming from scheduler (create invoice)
        ...
        } else if (invoiceData && isEditMode) {
            // Handle data coming from ViewAllInvoices (edit invoice)
            setCustomer_Name(invoiceData.customerName || "");
            setMobileNo(invoiceData.phoneNumber || "");
            setEmail(invoiceData.email || "");
            setAddress(invoiceData.address || "");
            setComments(invoiceData.comment || "");
            setIsGST(invoiceData.isGST || false);
            setGSTNumber(invoiceData.gst_number || "");
            setDiscount(invoiceData.discount || 0);

            console.log("qwetyuiopzdfxgchjgklcvbn", invoiceData);
            // Transform services
            const transformedServices = invoiceData.services.map((service) => ({
                id: service.id || service.service_id, // Ensure ID is consistent
                name: service.Description || service.name,
                price: service.Price || 0,
                quantity: service.Quantity || 1,
                discount: service.Discount || 0,
                staff: service.Staff ? [{ label: service.Staff }] : [],
            ...
            setService(transformedServices);
            setServicesTableData(transformedServices);

            setSelectedList(transformedServices);
            ...
            // Set payment modes
            if (
                invoiceData.paymentModes &&
                invoiceData.paymentModes.length > 0
            ) {
                const modes = {};
                invoiceData.paymentModes.forEach((mode) => {
                    modes[mode.mode] = mode.amount;
                });
                setPaymentModes(modes);
            }
        }
    }, [appointment, invoiceData, isEditMode]);

    // Payment mode handlers
    const handleSelectChange = (selectedOptions) => {
        const updatedPayments = {};
        selectedOptions.forEach((option) => {
            updatedPayments[option.value] = paymentModes[option.value] || "";
        });
    ...
    // State for payment modes and discount
    const [paymentModes, setPaymentModes] = useState(
        invoiceData?.paymentModes?.reduce((acc, mode) => {
            acc[mode.mode] = mode.amount;
            return acc;
        }, {}) || {}
    );
    ...
    console.log(
        "Modal Service IDs:",
        categoryServices.flatMap((c) => c.services.map((s) => s.id))
    );
    return (
        <>
            <div className="bg-gray-100">
                ...
                <div className="bg-gray-100 min-h-[150vh] md:ml-72 p-10">
                    {userExists ? (
                        <CustomerSummary
                            dataPromise={Promise.resolve(customerId)}
                            mobile_no={mobile_no}
                            bid={bid}
                        />
                    ) : null}
                    ...
                    <div className="bg-white max-h-full shadow-md p-12 mt-10 rounded-[2.5rem] ">
                        ...
                        <form onSubmit={handleGenerateInvoice}>
                            ...
                            <div className="flex flex-wrap gap-4 lg:gap-12 mt-12 overflow-auto">
                                <div>
                                    ...
                                    <div
                                        className="my-4 overflow-auto rounded-[2.5rem] "
                                        id="service-table"
                                    >
                                        {combinedList.length > 0 ? (
                                            <table className="w-full border border-gray-200 ">
                                                        ...
                                                        (combine, index) => (
                                                            ...
                                                            >
                                                                ...
                                                                <td className="p-2 border">
                                                                    <input
                                                                        type="number"
                                                                        className="border w-20 px-2 py-1 rounded-full text-center"
                                                                        placeholder="Price"
                                                                        min="0"
                                                                        value={
                                                                            combine.gst ===
                                                                ...
                                                                <td className="p-2 border">
                                                                    {combine.stock ? ( // This is a product with stock information
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                onChange={(
                                                                                    ...
                                                                                    if (
                                                                                        ...
                                                                                        alert(
                                                                                            `Only ${combine.stock} available in stock`
                                                                                        );
                                                                                        return;
                                                                                    }
                                                                                    handleInputChange(
                                                                ...
                                                                <td className="p-2 border">
                                                                    <select
                                                                        className="border px-2 rounded-full py-1"
                                                                        value={
                                                                            combine.gst ||
                                                                            "No GST"
                                                                        }
                                                                ...
                                                                >
                                                                    <div
                                                                        className="border px-2 py-1 bg-white  text-black rounded-full cursor-pointer"
                                                                        onClick={() =>
                                                                            openModal(
                                                                                index
                                                                            )
                                                                    ...
                                                                    >
                                                                        ...
                                                                        combine
                                                                            ...
                                                                            <span>
                                                                              ...
                                                                              combine
                                                                          ) ? ( // Check if service exists in selectedList
                                                ...
                                                <tfoot className="bg-blue-50 text-blue-700">
