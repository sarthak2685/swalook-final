...
const ExpenseSetting = () => {
    ...
    const handleAddCategory = () => {
        setCategories([...categories, { id: categories.length + 1, value: "", isCustom: false }]);
    };

    const handleCategoryChange = (id, value) => {
        setCategories(
            categories.map((category) =>
                category.id === id ? { ...category, value, isCustom: !predefinedCategories.includes(value) } : category
            )
        );
    };
    ...
    const handleSaveCategories = async () => {
        ...
        try {
            ...
            console.log(token)
            const response = await fetch(`${config.apiUrl}/api/swalook/expense_category/?branch_name=${bid}`, {
                method: "POST",
                headers: {
