...
const SalarySlipPopup = ({ onClose, staffId }) => {
  ...
  const SalarySlipDocument = ({ employeeData, netPay, currentMonth, currentYear }) => {
       ...
       <Page style={styles.page}>
        ...
        <View style={styles.row}>
          <Text>
            ...
            <Text style={styles.fieldValue}>{employeeData?.staff_joining_date || "N/A"}</Text>
          </Text>
          <Text>
            <Text style={styles.fieldLabel}>Employee Name: </Text>
            <Text style={styles.fieldValue}>{employeeData?.staff_name || "N/A"}</Text>
          </Text>
        </View>
        <View style={styles.row}>
          <Text>
            <Text style={styles.fieldLabel}>Payslip Period: </Text>
            <Text style={styles.fieldValue}>{currentMonth} {currentYear}</Text>
          </Text>
          <Text>
            <Text style={styles.fieldLabel}>Designation: </Text>
            <Text style={styles.fieldValue}>{employeeData?.staff_role || "N/A"}</Text>
          </Text>
        </View>
        <View style={styles.row}>
          <Text>
            <Text style={styles.fieldLabel}>Worked Days: </Text>
            <Text style={styles.fieldValue}>{netPay?.no_of_working_days || 0}</Text>
          </Text>
        </View>
          {/* Earnings Table */}
          <View style={styles.tableContainer}>
            <View style={styles.tableHeader}>
              ...
              <Text style={[styles.tableColumn, styles.lastColumn]}>Amount</Text>
            </View>
  
            <View style={styles.tableRow}>
              <Text style={styles.tableColumn}>Basic</Text>
              <Text style={[styles.tableColumn, styles.lastColumn]}>
            ...
            {employeeData.staff_commision_cap > 0 && (
              <View style={styles.tableRow}>
                <Text style={styles.tableColumn}>Commission</Text>
                <Text style={[styles.tableColumn, styles.lastColumn]}>
            ...
            {employeeData.house_rent_allownance > 0 && (
              <View style={styles.tableRow}>
                <Text style={styles.tableColumn}>House Rent Allowance</Text>
                <Text style={[styles.tableColumn, styles.lastColumn]}>
                 Rs. {employeeData.house_rent_allownance || 0}
                </Text>
              </View>
            )}
            ...
            {employeeData.meal_allowance > 0 && (
              <View style={styles.tableRow}>
                <Text style={styles.tableColumn}>Meal Allowance</Text>
                <Text style={[styles.tableColumn, styles.lastColumn]}>
            ...
            <View style={[styles.tableRow, styles.totalRow]}>
              <Text style={styles.tableColumn}>Total Earnings</Text>
              <Text style={[styles.tableColumn, styles.lastColumn]}>
               Rs. {netPay?.earning || 0}
              </Text>
            </View>
          ...
          {(employeeData.staff_provident_fund > 0 || employeeData.staff_professional_tax > 0 || employeeData.totalDeductions > 0) && (
            <View style={styles.tableContainer}>
              <View style={styles.tableHeader}>
                <Text style={styles.tableColumn}>Deductions</Text>
                <Text style={[styles.tableColumn, styles.lastColumn]}>Amount</Text>
              </View>
  
              {employeeData.staff_provident_fund > 0 && (
                <View style={styles.tableRow}>
                  <Text style={styles.tableColumn}>Provident Fund</Text>
                  <Text style={[styles.tableColumn, styles.lastColumn]}>
              ...
              {employeeData.staff_professional_tax > 0 && (
                <View style={styles.tableRow}>
                  <Text style={styles.tableColumn}>Professional Tax</Text>
                  <Text style={[styles.tableColumn, styles.lastColumn]}>
                   Rs. {employeeData.staff_professional_tax}
                  </Text>
                </View>
              )}
              ...
              {employeeData.totalDeductions > 0 && (
                <View style={styles.tableRow}>
                  <Text style={styles.tableColumn}>Total Deductions</Text>
                  <Text style={[styles.tableColumn, styles.lastColumn]}>
          ...
          <Text>
            <Text style={styles.fieldLabel}>Net Pay in Words:</Text>{" "}
          ...
          <Text style={styles.disclaimer}>[This is a system-generated Salary Slip]</Text>
        </Page>
      </Document>
    );
  };
          ...
          <>
            {salarySlipData ? (
              <>
                    ...
                    {salarySlips.map((slip) => (
                      <tr key={slip.id}>
                        ...
                        <td>
                          {slip.downloadLink ? (
                            <PDFDownloadLink
                              document={<SalarySlipDocument employeeData={employeeData} netPay={netPay} currentMonth={currentMonth} currentYear={currentYear} />}
                              fileName={`Salary_Slip_${employeeData.staff_name}_${currentMonth}_${currentYear}.pdf`}
                            >
                              {({ loading }) => (loading ? "Loading document..." : "Download PDF")}
                            </PDFDownloadLink>
                          ) : (
                            <button onClick={() => handleDownloadSlip(slip.id)}>Generate PDF</button>
                          )}
                        </td>
                      </tr>
                    ))}
                ...
                <button className="generate-button2" onClick={handleGenerateSalarySlip}>Generate Salary Slip</button>
              </>
            ) : (
