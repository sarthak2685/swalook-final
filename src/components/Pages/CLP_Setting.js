...
const handleSaveEdit = async () => {
  ...
  const handleSaveEditedCoupon = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${config.apiUrl}/api/swalook/coupon/?id=${couponToEdit.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${localStorage.getItem('token')}`,
    ...
    <>
              ...
              <table className="min-w-full bg-white border rounded-lg">
                <thead>
                  <tr className="bg-gray-200 text-left text-gray-600">
                    ...
                    <th className="py-3 px-4 font-semibold text-xs sm:text-xl">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {fetchedRows.map((row) => (
                    <tr key={row.id} className="border-b">
                      ...
                      <td className="py-3 px-4 text-xs sm:text-xl text-gray-700">
                        {row.points_hold ? `${row.points_hold} P` : `${row.discount}%`}
                      </td>
                ...
                <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
                  ...
                  <div className="space-y-4">
                    <input
                      type="text"
                      name="program_type"
                      value={selectedRow?.program_type || ""}
                      onChange={(e) => handleEditChange("program_type", e.target.value)}
                      placeholder="Membership Name"
                      className="w-full border rounded-lg px-3 py-2"
                    />
                    <input
                    ...
                    <input
                      ...
                      value={selectedRow?.expiry_duration || ""}
                      onChange={(e) => handleEditChange("expiry_duration", e.target.value)}
                      placeholder="Expiry (in Months)"
                      className="w-full border rounded-lg px-3 py-2"
                    />
                    <select
                      name="benefit"
                      value={selectedRow?.benefit || ""}
                      onChange={(e) => handleEditChange("benefit", e.target.value)}
                      className="w-full border rounded-lg px-3 py-2"
                      required
                    >
                      <option value="">Select Benefit</option>
                    ...
                    {selectedRow?.benefit === "Points Balance" && (
                      <input
                        type="number"
                        name="points_hold"
                        value={selectedRow?.points_hold || ""}
                        onChange={(e) => handleEditChange("points_hold", e.target.value)}
                        placeholder="Points per Rs. 100 spent"
                        className="w-full border rounded-lg px-3 py-2"
                      />
                    )}
                    {selectedRow?.benefit === "Discount" && (
                      <>
                        <input
                          type="number"
                          name="discount"
              ...
              {showCouponModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                  <div className="bg-white rounded-lg w-full max-w-md p-6 relative">
                    ...
                    <h3 className="text-xl font-semibold mb-4 text-left">Coupon</h3>
                    <form onSubmit={handleSaveCoupon}>
                      <div className="mb-4">
                        <input
                          ...
                          onChange={(e) =>
                            setCouponFormData({ ...couponFormData, couponName: e.target.value })
                          }
                        />
                      </div>
                      <div className="mb-4">
                        <input
                          ...
                          value={couponFormData.charges}
                          onChange={(e) =>
                            setCouponFormData({
                              ...couponFormData,
                              charges: e.target.value.replace(/[^0-9]/g, ""),
                            })
                          }
                        />
                      </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-lg w-full max-w-md p-6 relative">
          <button
            className="absolute top-4 right-4 text-5xl hover:text-black text-red-700"
            onClick={handleCloseEditCouponModal}>
            &times;
          </button>
          <h3 className="text-xl font-semibold mb-4 text-left">Edit Coupon</h3>
          <form onSubmit={handleSaveEditedCoupon}>
            <div className="mb-4">
              <input
                type="text"
                className="w-full border border-gray-300 rounded-lg p-3 focus:outline-blue-500"
                placeholder="Enter coupon name"
                value={couponToEdit.coupon_name}
                onChange={(e) => setCouponToEdit({ ...couponToEdit, coupon_name: e.target.value })}
              />
            </div>
            <div className="mb-4">
              <input
                ...
                className="w-full border border-gray-300 rounded-lg p-3 focus:outline-blue-500"
                placeholder="Enter charges"
                value={couponToEdit.coupon_price}
                onChange={(e) =>
                  setCouponToEdit({ ...couponToEdit, coupon_price: e.target.value.replace(/[^0-9]/g, "") })
                }
              />
            </div>
            <div className="mb-4">
              <input
                ...
                className="w-full border border-gray-300 rounded-lg p-3 focus:outline-blue-500"
                placeholder="Enter balance"
                value={couponToEdit.coupon_points_hold}
                onChange={(e) =>
                  setCouponToEdit({ ...couponToEdit, coupon_points_hold: e.target.value.replace(/[^0-9]/g, "") })
                }
              />
            </div>
            <div className="mb-6 flex items-center">
              <input
                ...
                checked={couponToEdit.active}
                onChange={(e) => setCouponToEdit({ ...couponToEdit, active: e.target.checked })}
              />
              <label htmlFor="editActiveCheckbox" className="ml-2 text-gray-700 font-medium text-xl">
                Active
              </label>
            </div>
            <button
